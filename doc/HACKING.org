...
- Unit-level coverage now includes [[file:../src/__tests__/app.registerTools.spec.ts][App.registerTools]] and [[file:../src/__tests__/lsp-manager.spec.ts][LspManager selection]] suites. Run =yarn test= (Jest + ts-jest) locally before opening a PR.
- For new features, keep adding focused specs under =src/__tests__/= and mock [[file:../src/lsp.ts::21][=LspClient=]] to avoid spawning real servers.
...
- No automated gating exists, so record manual verification commands (Jest, MCP CLI flows) in PR descriptions.
